...
export function ChatView({ chat, sending, dark, onToggleTheme, onMenu, onNewVideo, onSend }: Props) {
  ...
  return (
    <main className="flex min-w-0 flex-1 flex-col bg-cream dark:bg-[#1c1a2c]">
      <header className="flex h-[72px] shrink-0 items-center gap-3 border-b border-black/[0.055] bg-cream/85 px-4 backdrop-blur-xl dark:border-white/[0.07] dark:bg-[#1c1a2c]/85 sm:px-6">
        <button onClick={onMenu} className="rounded-xl p-2 hover:bg-black/5 lg:hidden" aria-label="Open sidebar"><Menu size={20} /></button>
        <img src={chat.video.thumbnailUrl ?? ''} alt="" className="h-9 w-14 rounded-lg object-cover" />
        <div className="min-w-0 flex-1">
          <h1 className="truncate text-sm font-bold">{chat.video.title}</h1>
          <div className="mt-1 flex items-center gap-1.5 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400"><CheckCircle2 size={11} /> Ready · {chat.video.chunkCount} source chunks</div>
        </div>
        <button onClick={onToggleTheme} className="rounded-xl border border-black/[0.06] bg-white p-2.5 text-slate-500 hover:text-violet dark:border-white/10 dark:bg-white/5 dark:text-slate-300" aria-label="Toggle theme">{dark ? <Sun size={17} /> : <Moon size={17} />}</button>
        <button onClick={onNewVideo} className="hidden items-center gap-2 rounded-xl border border-black/[0.06] bg-white px-3.5 py-2.5 text-xs font-bold hover:border-violet/30 hover:text-violet dark:border-white/10 dark:bg-white/5 sm:flex"><Plus size={15} /> New video</button>
      </header>
      ...
      <div className="shrink-0 border-t border-black/[0.05] bg-cream/90 px-4 py-4 backdrop-blur-xl dark:border-white/[0.06] dark:bg-[#1c1a2c]/90 sm:px-8 sm:py-5">
        <div className="mx-auto max-w-4xl">
          ...
          <p className="mt-2 text-center text-[10px] text-slate-400">Enter to send · Shift+Enter for a new line · AI can make mistakes—check the linked sources.</p>
        </div>
      </div>
    </main>
  );
